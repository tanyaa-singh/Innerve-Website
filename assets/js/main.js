...
var tag, tv, vid, randomVid, currVid, firstScriptTag, createdVideoDiv, ytLoaded = 0,
    playerDefaults = {
        autoplay: 1,
        autohide: 1,
        modestbranding: 0,
...
if (typeof v_start !== "number") videoStartTime = 0;
else
    videoStartTime = v_start;
if (typeof v_end !== "number") videoEndTime = 999999;
else
    videoEndTime = v_end;
...
function onPlayerStateChange(e) {
    ...
    $(function() {
        ...
        function stopScrolling(action) {
            if (action === 1) $("body").addClass("stop-scrolling");
            else
        ...
        function setUserFormsPosition() {
            var screenHeight = viewport().height,
                formRecipient, formRecipientHeight = 0,
                passFormHeight = 0,
                reviewFormHeight = 0,
                topbarheight = 82,
                totalHeight = 0;
                ...
                if ($(".hero-header").length) {
                    ...
                    if (viewport().width < 768) {
                        if (!_.hasClass("search-popup")) {
                            ...
                            _.find(".hero-search").stop().animate({
                                marginTop: "40px"
                            }, {
                                duration: 1000
                            });
        ...
        function filterButtonMaxWidth() {
            ...
            if (viewport().width >= 480) {
                ...
                $(".listing-results .bootstrap-select").css({
                    "min-width": "auto",
                    "max-width": v
                });
            } else {
        ...
        function bodyWrapperOverflow() {
            if (isMobile()) $(".body-wrapper").addClass("is-mobile");
            else
                    ...
                    if (local_video !== undefined && local_video !== "" && local_video !== " ") {
                        ...
                        $(".hero-header").each(function() {
                            if ($(this).hasClass("h-video")) {
                                ...
                                $(".hero-video").css({
                                    opacity: 1
                                }).append('<div id="video-container"><video id="background_video" loop muted></video><div id="video_cover"></div>');
                                var bv = new Bideo();
                                bv.init({
                        ...
                        if ($('#video-container').css("opacity") === "0") {
                            ...
                            $('#video-container').stop().delay(600).animate({
                                opacity: 1
                            }, {
                                duration: 3000
                            });
        ...
        function vmVidRescale() {
            if (typeof vm_video !== "undefined") {
                if (vm_video !== undefined && vm_video !== "" && vm_video !== " ") {
                    ...
                    if ($(".hero-video").css("opacity") === "0") {
                        $(".hero-video").css("opacity", "0.01");
                        $(".hero-video").stop().delay(2000).animate({
                            opacity: 1
                        }, {
                            duration: 3000
                        });
        ...
        var objectFitVideos = function() {
            ...
            function fitIt($el, style) {
                ...
                function doWork() {
                    ...
                    if (videoRatio < wrapRatio ? style['object-fit'] === 'contain' : style['object-fit'] === 'cover') {
                        ...
                        else if (style['object-position-x'] === 'right') setCss.marginLeft = Math.round(wrapWidth - newHeight) + 'px';
                        else
                        ...
                        else if (style['object-position-y'] === 'bottom') setCss.marginTop = Math.round(wrapHeight - newWidth) + 'px';
                        else
            ...
            function parsePosition(style) {
                if (~style['object-position'].indexOf('left')) style['object-position-x'] = 'left';
                else if (~style['object-position'].indexOf('right')) style['object-position-x'] = 'right';
                else
                    style['object-position-x'] = 'center';
                if (~style['object-position'].indexOf('top')) style['object-position-y'] = 'top';
                else if (~style['object-position'].indexOf('bottom')) style['object-position-y'] = 'bottom';
                else
        ...
        function checkHeroFeatPosts() {
            setTimeout(function() {
                if (!$(".hero-header .feat-posts").length) {
                    ...
                    if (viewport().width > 767) {
                        $(".hero-search,.search-categories").stop().animate({
                            width: hs * 0.8,
                            left: hs * 0.1
                        });
                    } else {
        ...
        function showColors() {
            ...
            var colors = $(".template-colors a").length,
                colorCounter = 0;
            colorsContainer.animate({
                height: colorsContainer[0].scrollHeight
            }, {
                duration: 2000,
                complete: function() {
                    colorsContainer.css({
        ...
        if ($(".search-categories .categories").length)
            if ($(".search-categories .categories")[0].scrollHeight > 80) $(".search-categories .more-categories").css({
                display: "block"
            });
        ...
        $(".search-cities-toggle").delay(3000).animate({
            opacity: 1
        });
        if ($(".gallery").length || $("a[data-lightbox='gallery']").length) $(".lightbox").css({
            marginLeft: 0
        });
        ...
        if (!$("header").length) $("body").append('<div class="full-dimming-overlay"></div>');
        else
        ...
        var landing = setInterval(function() {
            if ($(".h-video.landing").height() !== 500) {
                $(".hero-header.landing .header-centralizer").css({
                    height: "100%"
                });
                $("body").css({
                    width: "99.9%"
        ...
        $('body').on("click", ".search-cities a", function(e) {
            ...
            if (!$(".hero-header").hasClass("open-cities-list")) {
                ...
                } else if ($(e.target).is(".go-more-cities") || $(e.target).parent().is(".go-more-cities")) {} else {
                    e.preventDefault();
                    e.stopPropagation();
        ...
        $(".background-caption").on({
            ...
            'mouseleave': function(e) {
                var _ = $(this).find(".bg-title");
                $(".hero-arrow").removeClass("hover").stop().animate({
                    left: 10,
                    bottom: 45
                }, {
                    duration: 300
        ...
        $(".search-categories .more-categories").on({
            'click': function(e) {
                ...
                $(".search-categories .minus-categories").css({
                    display: "block"
                });
                $(".search-cities-toggle,.search-cities").css({
                    display: "none"
                });
                videoRescale();
                resizeCanvases();
        ...
        $(".search-categories .minus-categories").on({
            'click': function(e) {
                ...
                if (_.height() > 120) {
                    ...
                    $(".search-categories .more-categories").css({
                        display: "block"
                    });
                } else {
                    __.stop().animate({
                        marginTop: parseInt(__.css('margin-top').replace(/[^-\d\.]/g, ''), 10) + 76
                    });
        ...
        $("#slide-nav").on("click", toggler, function(e) {
            ...
            if (!selected) {
                ...
                $(".full-dimming-overlay").stop().animate({
                    opacity: 0
                }, {
                    duration: 1000,
                    complete: function() {
        ...
        $(".listing-item-link").on({
            "mouseenter": function() {
                ...
                if (_.parent().parent().parent().hasClass("grid2") || _.parent().parent().parent().hasClass("grid3")) {
                    ...
                    _.siblings('.listing-item-author').stop().animate({
                        top: "110px",
                        opacity: 0
                    });
                    _.siblings('.listing-item-date').stop().animate({
                        top: "-60px",
                        opacity: 0
                    });
                    ...
                    _.find('.listing-item-rating').stop().animate({
                        top: "-20px",
                        opacity: 0
                    });
                    _.siblings('.listing-item-author').stop().animate({
            ...
            "mouseleave": function() {
                var _ = $(this);
                if (_.parent().parent().parent().hasClass("grid2") || _.parent().parent().parent().hasClass("grid3")) {
                    _.siblings('.listing-item-rating').stop().animate({
                        top: "10px",
                        opacity: 1
                    }, {
                        complete: function() {
                            _.siblings('.listing-item-rating').css({
                                top: ""
                            });
                        }
                    });
                    _.siblings('.category-icon').stop().animate({
                        top: "25px",
                        opacity: 1
                    }, {
                        complete: function() {
                            _.siblings('.category-icon').css({
                                top: ""
                            });
                        }
                    });
                    _.siblings('.listing-item-author').stop().animate({
                        ...
                        complete: function() {
                            _.siblings('.listing-item-author').css({
                                top: ""
                            });
                        }
                    });
                    _.siblings('.listing-item-date').stop().animate({
                        ...
                        complete: function() {
                            _.siblings('.listing-item-date').css({
                                top: ""
                            });
                        }
                    });
                    ...
                    _.find('.listing-item-rating').stop().animate({
                        ...
                        complete: function() {
                            _.find('.listing-item-rating').css({
                                top: ""
                            });
                        }
                    });
                    _.siblings('a').not(".listing-item-author").stop().animate({
                        ...
                        complete: function() {
                            _.siblings('a').not(".listing-item-author").css({
                                top: ""
                            });
                        }
                    });
                    ...
                    _.siblings('a, .listing-item-rating').not(".listing-item-author").stop().animate({
                        ...
                        complete: function() {
                            _.siblings('a, .listing-item-rating').not(".listing-item-author").css({
                                top: ""
                            });
                        }
                    });
        ...
        $(".gallery-item a").on("mouseenter", function() {
            ...
            $(this).parent().find(".gallery-item-caption").stop().animate({
                opacity: 0
            }, {
                duration: 1000
            });
        ...
        $("body").on("click", ".header-search-button,.search-pop-button", function(e) {
            ...
            if (!$(".search-popup").hasClass("showing-search")) {
                stopScrolling(1);
                $(".search-popup").addClass("showing-search").css({
                    left: "-100%",
                    zindex: 999
                }).stop().animate({
                    left: 0,
                ...
                $(".review-popup").stop().animate({
                    left: "-200%",
                    opacity: 0.99
                }, {
                    duration: 500
                });
                $(".pm-popup").stop().animate({
                    left: "-200%",
                    opacity: 0.99
                }, {
                    duration: 500
        ...
        $(".quick-menu a").on("click", function(e) {
            ...
            if (_.hasClass("pvt")) {
                ...
                stopScrolling(1);
                $(".pm-popup").addClass("showing-pm").css({
                    left: "-100%"
                }).stop().animate({
                    left: 0,
                    opacity: 1
        ...
        $("body").on("click", ".user-login,.sign-in", function(e) {
            ...
            if (!$(".login-popup").hasClass("showing-login")) {
                stopScrolling(1);
                $(".login-popup").addClass("showing-login").css({
                    left: "-100%",
                    zIndex: 999
                }).stop().animate({
                    left: 0,
        ...
        function setMapDefaultVisibility() {
            if ($("body").hasClass("map-and-page")) {
                ...
                $(".listing-block, footer").css({
                    display: "block"
                });
            } else {
                $(".listing-block").stop().animate({
                    opacity: 0
                }, {
                    duration: 500,
                    complete: function() {
            ...
            if ($(".mapList").length) {
                if (viewport().width > 767) {
                    var listRight = parseInt($(".mapList").css("right").replace(/[^-\d\.]/g, ''), 10),
                        ww = $(window).width(),
                        dif;
                    if (hasVerticalScrollbar()) dif = 270;
                    else
                        dif = 285;
        ...
        if (typeof mapMarkers !== "undefined" && $("#map").length) {
            ...
            for (var i = 0; i < mapMarkers.length; i++) {
                ...
                var icon;
                if (typeof templistingItem.icon === "undefined") icon = "icon-map-marker";
                else
                ...
                markers[templistingItem.id] = new L.marker(latLng, {
                    icon: divMarker
                }).addTo(map);
                ...
                if ($(".mapList").length) {
                    ...
                    if (typeof templistingItem.link !== "undefined") $(".temp-div").append('<a href="' + templistingItem.link + '"></a>');
                    else
        ...
        $("body").on("click", ".back-listing-button", function() {
            ...
            $(".page-head,.map-buttom").stop().animate({
                opacity: 1
            });
            $(".map-listing").stop().animate({
                height: 250
        ...
        $("body").on("click", ".leaflet-div-icon", function(e) {
            ...
            if (typeof listingItem[id].link !== "undefined") {
                if (listingItem[id].link !== "undefined" && listingItem[id].link !== undefined && listingItem[id].link !== "#" && listingItem[id].link !== "" && listingItem[id].link != " ") html = '<div class="maplink ' + id + '" data-link="' + listingItem[id].link + '">';
                else {
        ...
        $("body").on("click", ".pop-coor-icon", function(e) {
            e.preventDefault();
            e.stopPropagation();
            if ($(this).parent().siblings('.pop-coors').css("display") === "none") $(this).parent().siblings('.pop-coors').css({
                display: "block"
            });
            else
                $(this).parent().siblings('.pop-coors').css({
                    display: "none"
                });
        });
        ...
        $(window).on("resize", function() {
            if (typeof mapMarkers !== "undefined" && $("#map").length) {
                if (!$(".map-listing.map-hidden").length) $(".map-listing").stop().animate({
                    height: $(window).height() - 60
                }, {
                    duration: 300
                });
